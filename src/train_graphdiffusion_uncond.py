...
def setup_logger(args):
    logger = logging.getLogger(f"Diffusion_{args.dataset}_{args.loss_type}_uncond")
    handler = logging.FileHandler(filename=f"../logs/Diffusion_Log/Uncond/Diffusion_{args.dataset}_{args.loss_type}_uncond.log", mode="w")
...
if __name__ == "__main__":
    ...
    logger.info("===================================================")
    logger.info(f"========   Graph Diffusion with no Classs Info  ===============")
    logger.info("===================================================")
    ...
    for epoch in range(1, args.diff_epochs):
        ...
        if diff_loss.item() < best_loss:
            ...
            model.save_model(args, name=f"GenGA_{args.dataset}_{args.loss_type}_Uncond")
